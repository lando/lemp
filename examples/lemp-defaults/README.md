...
lando ssh -s appserver_nginx -c "curl http://localhost/info.php" | grep "memory_limit" | grep "1G"
...
lando ssh -s appserver -c "cat /usr/local/etc/php/conf.d/zzz-lando-my-custom.ini" | grep "; LANDOLEMPPHPINI"
lando ssh -s appserver_nginx -c "curl -L http://localhost/info.php" | grep max_execution_time | grep 91
lando ssh -s database -c "cat /opt/bitnami/mysql/conf/my_custom.cnf" | grep "LANDOLEMPMYSQLCNF"
