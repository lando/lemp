...
Lando offers a configurable [recipe](https://docs.lando.dev/config/recipes.html) for developing [LEMP](https://drupal.org/) apps.
...
The [recipe config](https://docs.lando.dev/config/recipes.html#config) to set the LEMP recipe to use `php` version `5.4` is shown below:
...
Note that the default files may change based on how you set both `ssl` and `via`. Also note that the `vhosts` and `server` config will be explicitly for `nginx`. We *highly recommend* you check out the [nginx](./nginx.html#configuration) if you plan to use a custom `vhosts` or `server` config.
